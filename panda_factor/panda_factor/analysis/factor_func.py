...
本模块提供了因子分析过程中使用的各种工具函数，就像一个"因子分析工具箱"，
它包含了从数据处理到指标计算的各种函数。

核心概念
--------

- **后复权价格**：根据分红配股调整的历史价格，用于计算真实收益率
- **未来收益率**：股票在未来 1/3/5/10/20/30 天的收益率
- **IC（信息系数）**：因子值与未来收益率的相关系数，衡量因子的预测能力
- **IR（信息比率）**：IC 均值除以 IC 标准差，衡量因子预测能力的稳定性
- **分位数**：将股票按因子值分组，分析不同组的收益表现

为什么需要这个模块？
-------------------

在因子分析中，需要进行大量的数据处理和指标计算：
- 计算后复权价格和未来收益率
- 计算因子的 IC、IR 等指标
- 对因子值进行分组和极值处理
- 生成分析报告和图表
...
1. **数据处理工具**：计算后复权价格、未来收益率等（就像数据清洗）
2. **指标计算工具**：计算 IC、IR、分位数等（就像统计分析）
3. **极值处理工具**：处理因子中的异常值（就像数据清洗）
4. **分组工具**：将股票按因子值分组（就像分类）

主要函数
--------

数据处理：
- `cal_hfq`: 计算后复权价格和未来收益率
- `cal_pct_lag`: 计算滞后收益率

指标计算：
- `cross_section_ic`: 计算横截面 IC
- `information_ratio`: 计算信息比率

极值处理：
- `winsorize_by_median`: 中位数法去极值
- `winsorize_by_std`: 标准差法去极值
...
- 输入数据通常需要包含特定的列（如 date、symbol、close 等）
- 某些函数需要足够的历史数据才能准确计算
- 函数返回的结果通常是 DataFrame 或 Series
"""
